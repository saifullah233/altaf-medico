...
function orderViaWhatsApp() {
  ...
  const lines = cart.items.map(i => `• ${i.name} x${i.qty} — ₹${(i.price * i.qty).toFixed(2)}`).join('\n');
  const msg = `Hello Altaf Medical! I would like to order:\n\n${lines}\n\nTotal: ₹${cart.total.toFixed(2)}\n\nPlease confirm availability.`;
  window.open(`https://wa.me/918210692437?text=${encodeURIComponent(msg)}`, '_blank');
}
...
const SHOP_CAT_ALIAS = { rx: 'prescription', vitamins: 'vitamins', otc: 'otc', diabetes: 'diabetes', baby: 'baby', surgical: 'surgical', personal: 'personal' };
...
function initShopFilters() {
  ...
  document.querySelector('#sortSelect')?.addEventListener('change', applyShopFilters);
  document.querySelector('#shopSearch')?.addEventListener('input', applyShopFilters);
}

function applyShopFilters() {
  const q = document.querySelector('#shopSearch')?.value.toLowerCase() || '';
  const selectedCats = [...document.querySelectorAll('.filter-check input:checked')].map(i => i.value);
...
function switchTab(tabId, btn) {
  ...
  scope.querySelectorAll('.pd-tab-btn, .tab-btn').forEach(b => b.classList.remove('active'));
  scope.querySelectorAll('.pd-tab-pane, .tab-pane').forEach(p => p.classList.remove('active'));
  btn?.classList.add('active');
  (scope.querySelector(`#tab-${tabId}`) || document.querySelector(`#tab-${tabId}`))?.classList.add('active');
}
...
function initRequestForm() {
  ...
  form?.addEventListener('submit', e => {
    ...
    window.open(`https://wa.me/918210692437?text=${encodeURIComponent(waMsg)}`, '_blank');
...
function initPrescriptionForm() {
  ...
  form.addEventListener('submit', e => {
    ...
    const urgency = form.querySelector('[name=rxUrgency], #rxUrgency')?.value || 'Normal';
...
function initSpecialOrderForm() {
  ...
  form.addEventListener('submit', e => {
    ...
    if (!name || !phone || !medicine) { showToast('Please fill in name, phone and medicine name.'); return; }
...
function initProductQty() {
  ...
  window.addToCartFromProduct = function() {
    ...
    const name  = document.querySelector('.pd-title')?.textContent?.trim() || 'Medicine';
...
function initCartEvents() {
  ...
  document.querySelector('.checkout-whatsapp')?.addEventListener('click', orderViaWhatsApp);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.why-card, .review-card, .category-card').forEach(el => {
    el.style.opacity = '0';
